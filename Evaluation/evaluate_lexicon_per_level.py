...
def evaluate_lexicon(weight_files, output_file):
    ...
    with open(output_file, "w", encoding="utf-8") as out:
        out.write("Subject\tLevel1_N_Rules\tLevel1_N_Words\tLevel1_averaged\tLevel2_N_Rules\tLevel2_N_Words\tLevel2_averaged\tLevel3_N_Rules\tLevel3_N_Words\tLevel3_averaged\tLevel4_N_Rules\tLevel4_N_Words\tLevel4_averaged\n")
    ...
    for file in weight_files:
        ...
        processed_lex = dict()
        levels = list(words.keys())
        for l in levels:
            processed_lex[l] = process_rules(mappings[l], complete_rule_set)
        ...
        l4_n_rules = 0
        try:
            for word in processed_lex[4]:
                l4_n_words += 1
                for rule in processed_lex[4][word]:
                    l4_n_rules += 1
            l4_aver = l4_n_rules / l4_n_words
            l4_aver = round(l4_aver,2)
        except:
            l4_aver = "NA"
            l4_n_words = "NA"
            l4_n_rules = "NA"
        ...
        with open(output_file, "a", encoding="utf-8") as out:
            out.write(subject + "\t" + str(l1_n_rules) + "\t" + str(l1_n_words) + "\t" + str(l1_aver) + "\t"
                      ...
                      + str(l4_n_rules) + "\t" + str(l4_n_words) + "\t" + str(l4_aver) + "\n")
...
if __name__ == "__main__":
    evaluate_lexicon([("./weights_csv/Bob_weights.csv", "P1_Eng"), ("./weights_csv/Etienne_weights.csv", "P2_Eng"), ("./weights_csv/Patty S1_weights.csv", "P3_Ger"),
                      ("./weights_csv/Katharina_weights.csv", "P4_Sn"), ("./weights_csv/Luisa_weights.csv", "P5_Esp"), ("./weights_csv/Valentin_weights.csv", "P6_Eng")]
                     , "rules_per_level_six_part.csv")
